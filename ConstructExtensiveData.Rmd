---
title: "Construct Event Study Data"
author: "Ellie Cox"
date: "2022-11-01"
...
```{r loaddata}
load("/Users/elizabeth/Documents/Capstone Paper/RightHandSide.Rdata") # created in previous file
...
#hrs - intervalled
controls$hrs <- controls$HRSWORK1
controls$hrs[which(controls$YEAR < 1980)] <- intervalled_hrsworked[which(controls$YEAR < 1980)]

## Filter fishy data
controls <- controls %>% 
  #filter(hrs < 98)
  filter(EMPSTAT == 1 | EMPSTAT == 2 | EMPSTAT == 3) #
```


```{r summarise}
# Calculating "mean" in this chunk achieves the same thing as summing the total in a group and dividing by the population size. I double checked this as well.
controls_county_year <- controls %>%
  group_by(YEAR, FIPS) %>% 
...
Data_out_filtered <- Data_out %>% filter(YEAR == "1960"| YEAR == "1970" | YEAR == "1980" | YEAR == "1990" | YEAR == "2000" | YEAR == "2001" | YEAR == "2002" | YEAR == "2003" | YEAR == "2004" | YEAR == "2005")
...
Data_with_Labor <- left_join(Data_with_Labor, Intensity_of_treatment, by.x = "FIPS",
                             by.y = "FIPS")
# Drops non matches
# All 51 samples from ACS in 2001-2004 because the data doesn't give county fips codes
# obviously drops data before 1976 when water data doesn't exist. BUT we don't want this to drop
Data_with_Labor <- Data_with_Labor[!(is.na(Data_with_Labor$Treatment_Intensity)),]
```

## For robustness check
Limit data only to counties that had violations in 1985.

```{r robustness}
data_robust <- Data_with_Labor %>% filter(Treatment_Intensity > 0)
```

## Write to .dta

```{r writedta}
write.dta(data = as.data.frame(Data_with_Labor), file = "/Users/elizabeth/Documents/Capstone Paper/EventStudyData_All.dta")
```

## Write to .Rdata

```{r writeRdata}
save(Data_with_Labor, file = "/Users/elizabeth/Documents/Capstone Paper/DataForRegression_All.Rdata")
```

## Write to .dta

```{r writeRdata}
write.dta(data = as.data.frame(data_robust), file = "/Users/elizabeth/Documents/Capstone Paper/RobustData.dta")
```
